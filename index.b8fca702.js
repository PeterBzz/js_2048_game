...
game.getScore(); // Write your code here
 ...
 */ class Game {
   ...
   */ constructor(initialState = [
                ...
                return {
                    ...
                    setValue: (newValue)=>{
                        return cellsPromise.then((cells)=>{
                            if (this.board[rowIndex][colIndex].value !== 0) this.board[rowIndex][colIndex].cell.classList.remove(`field-cell--${this.board[rowIndex][colIndex].value}`);
                            this.board[rowIndex][colIndex].value = newValue;
                            this.board[rowIndex][colIndex].cell = cells[rowIndex * row.length + colIndex];
                            this.board[rowIndex][colIndex].cell.innerHTML = newValue;
                            if (newValue === 0) this.board[rowIndex][colIndex].cell.innerHTML = "";
                            this.board[rowIndex][colIndex].cell.classList.add(`field-cell--${newValue}`);
                            return this.board;
                        });
                    }
                };
        ...
        for(let i = 0; i < this.board.length; i++)for(let j = 0; j < this.board[i].length; j++){
            if (!this.boardTransposed[j]) this.boardTransposed[j] = [];
    ...
    move(isHorizontal, isRight) {
        ...
        if (!isHorizontal) board = this.boardTransposed;
        for(let i = 0; i < board.length; i++){
            const filteredRow = board[i].filter((cell)=>{
                return cell.value !== 0;
            });
            if (isRight) filteredRow.reverse();
            for(let j = 0; j < board[i].length; j++){
                let index = j;
                if (isRight) index = board[i].length - 1 - j;
                if (j < filteredRow.length - 1 && filteredRow[j + 1].value === filteredRow[j].value) {
    ...
    beforeMove() {
        if (this.state === Game.states.idle) this.switchState(Game.states.playing);
    }
    async afterMove() {
        await this.cellsPromise;
        if (this.board.flat(Infinity).some((cell)=>{
            return cell.value === 2048;
        })) this.switchState(Game.states.win);
        this.addNumber();
        this.checkLose().then((isLost)=>{
            if (isLost) this.switchState(Game.states.lose);
        });
    }
    async checkLose() {
        await this.cellsPromise;
        if (this.board.flat(Infinity).find((cell)=>{
            return cell.value === 0;
        })) return false;
        if (this.checkMoveAvailable(true)) return false;
        if (this.checkMoveAvailable(false)) return false;
        return true;
    }
    checkMoveAvailable(isHorizontal) {
        ...
        for(let i = 0; i < board.length; i++){
            ...
            if (filteredRow.length < board[i].length) return true;
            for(let j = 0; j < board[i].length; j++){
                if (j < filteredRow.length - 1 && filteredRow[j + 1].value === filteredRow[j].value) return true;
            }
        }
        return false;
    }
   ...
   */ getScore() {
        return this.board.reduce((sum, row)=>{
            return sum + row.reduce((sumRow, cell)=>{
   ...
   */ switchState(newState) {
        if (!Object.keys(Game.states).includes(newState)) return;
        switch(this.state){
            case Game.states.idle:
                document.querySelector(".start").removeEventListener("click", this.boundStart);
                document.querySelector(".message-start").classList.add("hidden");
                break;
            case Game.states.playing:
                document.querySelector(".restart").removeEventListener("click", this.boundRestart);
                break;
        ...
        switch(newState){
            case Game.states.idle:
                ...
                document.querySelector(".restart").classList.replace("restart", "start");
            ...
            case Game.states.playing:
                ...
                document.querySelector(".restart").addEventListener("click", this.boundRestart);
   ...
   */ addNumber() {
        ...
        let newNum = 2;
        if (Math.floor(Math.random() * 10) === 0) newNum = 4;
